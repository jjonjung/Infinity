# Infinity 문서 안내

## 먼저 볼 문서
- 백엔드 아키텍처: [architecture-v3-with-mfc-ops.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/architecture/architecture-v3-with-mfc-ops.md)
- 서버 구현 현황: [server-implementation-roadmap.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/server-implementation-roadmap.md)
- 데이터베이스 스키마: [auth_match_schema.sql](/C:/Users/EJ/Desktop/Fork/Infinity/docs/sql/auth_match_schema.sql)

## 폴더 구성
- 아키텍처: `/docs/architecture`
- MFC 관련 문서: `/docs/mfc`
- 알고리즘: `/docs/algorithms`
- SQL: `/docs/sql`

## 권장 읽기 순서
1. [architecture-v3-with-mfc-ops.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/architecture/architecture-v3-with-mfc-ops.md)
2. [server-implementation-roadmap.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/server-implementation-roadmap.md)
3. [mfc-admin-test-client-spec.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/mfc/mfc-admin-test-client-spec.md)
4. [mfc-test-client-core.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/mfc/mfc-test-client-core.md)
5. [auth_match_schema.sql](/C:/Users/EJ/Desktop/Fork/Infinity/docs/sql/auth_match_schema.sql)

## 문서별 용도
- [dedicated-server-auth-architecture.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/architecture/dedicated-server-auth-architecture.md)
  - 전용 서버, 인증 서버, MySQL 분리 구조를 처음 정리한 초안
- [architecture-v2-production.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/architecture/architecture-v2-production.md)
  - 인증, 매치, Redis 기준의 운영형 백엔드 구조를 정리한 문서
- [architecture-v3-with-mfc-ops.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/architecture/architecture-v3-with-mfc-ops.md)
  - MFC 모니터링과 테스트 도구까지 포함한 현재 목표 아키텍처
- [server-implementation-roadmap.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/server-implementation-roadmap.md)
  - 현재 코드 기준으로 어디까지 구현되었는지 정리한 문서
- [mfc-admin-test-client-spec.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/mfc/mfc-admin-test-client-spec.md)
  - MFC 모니터링 도구와 테스트 클라이언트의 역할을 정리한 문서
- [mfc-test-client-core.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/mfc/mfc-test-client-core.md)
  - 재사용 가능한 MFC 테스트 클라이언트 핵심 클래스 문서
- [mfc-dialog-skeleton.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/mfc/mfc-dialog-skeleton.md)
  - 다이얼로그 골격과 필요한 컨트롤 구성을 정리한 문서
- [이전 구현 자료구조 사용 개선.md](/C:/Users/EJ/Desktop/Fork/Infinity/docs/algorithms/이전%20구현%20자료구조%20사용%20개선.md)
  - 기존 Unreal 구현에서 자료구조 사용을 개선한 내용 정리
